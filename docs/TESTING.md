...
## Recette visuelle automatisée

`e2e/recette-visuelle.spec.ts` traverse vingt écrans — publics, intérimaire et
entreprise — en bureau et en mobile, et en rapporte une capture **pleine page**
par écran dans le dossier de sortie Playwright.

Elle n'affirme presque rien, à trois exceptions près, qui sont exactement les
défauts qu'une capture ne montre pas et qu'un relecteur ne pense pas à chercher :

- aucun **débordement horizontal** du document ;
- aucune **erreur JavaScript** non rattrapée sur l'ensemble du parcours ;
- le repère `main` présent et non vide sur chaque écran.

Elle a trouvé sa première régression le jour de son écriture : `main` était
devenu un élément flexible et refusait de descendre sous sa largeur min-content,
ce qui faisait défiler la page de travers sur un seul écran à 390 px. La cause,
elle, valait pour tous.

Les comptes viennent des fixtures de `src/scripts/test-server.ts` : les missions,
candidatures et profils traversés existent réellement dans le serveur éphémère.

    npx playwright test recette-visuelle --project=desktop
    npx playwright test recette-visuelle --project=mobile


## Contraste, mesuré et non supposé

`e2e/contraste.spec.ts` calcule les rapports de contraste WCAG réels : les
jetons du thème sont peints dans un canevas 1 × 1 puis relus en sRGB, parce que
les navigateurs conservent `oklch()` tel quel dans le style calculé — lire
`getComputedStyle().color` rendrait la chaîne, pas la couleur.

Il a trouvé deux défauts le jour de son écriture :

- du blanc sur `--color-clay` plafonnait à **3,7:1**. Les boutons d'accent
  utilisent désormais `--color-clay-ink` (**7,3:1**), et le test verrouille le
  fait que `--color-clay` reste une couleur d'aplat, jamais un fond de texte ;
- `--color-ink-faint` donnait **3,84:1** sur le papier, alors que c'est l'encre
  qui porte les textes les plus petits du produit. Elle a été assombrie
  (**5,04:1**).

    npx playwright test contraste --project=desktop
