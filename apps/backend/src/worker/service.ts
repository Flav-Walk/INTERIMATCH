...
export class WorkerService {
  ...
  private async ensureProfile(db: Db, id: string) {
    ...
    await db.query(
      "INSERT INTO worker_profiles(profile_id) VALUES($1) ON CONFLICT(profile_id) DO NOTHING",
      [id],
    );
  }
  ...
  private async recompute(db: Db, id: string) {
    ...
    await db.query(
      "UPDATE profiles SET onboarding_completed=$2, updated_at=now() WHERE id=$1",
      [id, complete],
    );
    return complete;
  }
  ...
  private async writePatch(
    ...
    if (columns.length)
      await db.query(
        `UPDATE worker_profiles SET ${columns.join(",")} WHERE profile_id=$1`,
        values,
      );
    ...
    if (names.length)
      await db.query(
        `UPDATE profiles SET ${names.join(",")}, updated_at=now() WHERE id=$1`,
        nameValues,
      );
  }
  ...
  async patch(id: string, patch: WorkerPatch) {
    ...
    await this.db.transaction(async (db) => {
      ...
      await this.recompute(db, id);
    });
  }

  async setSkills(id: string, skillIds: string[]) {
    await this.db.transaction(async (db) => {
      await this.writeSkills(db, id, skillIds);
      await this.recompute(db, id);
    });
  }
  ...
  async setCertifications(id: string, certifications: CertificationInput) {
    await this.db.transaction(async (db) => {
      await this.writeCertifications(db, id, certifications);
      await this.recompute(db, id);
    });
  }
  ...
  async updateAvailability(
    ...
    return this.db.transaction(async (db) => {
      ...
      const existing = (
        await db.query<{ starts_at: string; ends_at: string; status: string }>(
          "SELECT starts_at,ends_at,status FROM availabilities WHERE id=$1 AND profile_id=$2 FOR UPDATE",
          [slotId, id],
        )
      ).rows[0];
      if (!existing)
      ...
      const { rows } = await db.query(
        ...
        [slotId, id, starts, ends, patch.status ?? existing.status],
      );
      await this.recompute(db, id);
      return rows[0];
    });
