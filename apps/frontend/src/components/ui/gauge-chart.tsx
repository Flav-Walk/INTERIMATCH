/*
 * Gauge Chart — Animata
 * https://animata.design/docs/graphs/gauge-chart
 *
 * Une jauge en fer à cheval : un cercle SVG ouvert en bas (gap), dont l'arc
 * coloré se remplit jusqu'à la valeur. Tout est calculé en nombres dans le
 * SVG (rayon, périmètre, stroke-dashoffset) : pas de variables CSS, donc
 * rendu identique dans tous les navigateurs.
 *
 * Retouches InteriMatch :
 * 1. Chemin de cn() (pas de raccourci « @/ »).
 * 2. « transition-[stroke-dashoffset] ease-out » : Animata ne donnait que la
 *    durée, sans dire QUOI animer, donc l'arc sautait d'un coup.
 * 3. « Réduire les animations » : l'arc est dessiné directement plein.
 */
...
export default function GaugeChart({
  ...
  const adjustedProgress = shouldUseValue || reduceMotion ? progress : 0;
